...
# Kali Linux 是滚动发行版，默认已包含 main/non-free/contrib 源
...
# 复制本仓库内预编译的 anland_kde deb 包 (Kali 使用 Debian 系的 deb 包)
COPY anland-build/Kali/*.deb /tmp/anland-build/Kali/
...
RUN apt-get update && \
    ...
    if [ "$ENABLE_anland_kde_ARG" = "true" ] && ([ "$BUILD_KDE" = "min" ] || [ "$BUILD_KDE" = "conc" ] || [ "$BUILD_KDE" = "mobile" ]); then \
        ...
        dpkg -i /tmp/anland-build/Kali/*.deb || apt-get install -f -y && \
        echo "--> [开启] 设置预编译 deb 包为 hold 模式，防止被 apt 更新覆盖..." && \
        for f in /tmp/anland-build/Kali/*.deb; do \
...
RUN sed -i '/en_US.UTF-8/s/^# //' /etc/locale.gen && \
    ...
    # Kali Docker 镜像默认只有 root 用户，创建普通用户
    useradd -m -s /bin/bash ${USERNAME} && echo "${USERNAME}:1234" | chpasswd

# 为 Kali RootFS 安装 Droidspaces USB Manager
...
# Mesa 驱动适配
# Kali 基于 Debian Testing，使用与 Debian Trixie 相同的 Mesa 预编译包
