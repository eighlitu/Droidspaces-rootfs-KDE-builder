...
# 复制本仓库内预编译的 anland_kde deb 包 (Debian 13 预编译包兼容 Kali Rolling)
...
RUN apt-get update && \
    ...
    # 核心内核模块支持
    kmod tzdata \
    # Kali 核心渗透测试工具集 (kali-linux-headless)
    kali-linux-headless && \
    ############################################## KDE支持 ################################################
    # 最小化KDE
...
# 强制配置使用 iptables-legacy（兼容 Android 内核要求）
