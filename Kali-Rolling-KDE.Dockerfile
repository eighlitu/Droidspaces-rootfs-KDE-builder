...
COPY anland-build/KaliRolling/*.deb /tmp/anland-build/KaliRolling/
...
RUN apt-get update && \
    ...
    if [ "$ENABLE_anland_kde_ARG" = "true" ] && ([ "$BUILD_KDE" = "min" ] || [ "$BUILD_KDE" = "conc" ] || [ "$BUILD_KDE" = "mobile" ]); then \
        ...
        dpkg -i /tmp/anland-build/KaliRolling/*.deb || apt-get install -f -y && \
        echo "--> [开启] 设置预编译 deb 包为 hold 模式，防止被 apt 更新覆盖..." && \
        for f in /tmp/anland-build/KaliRolling/*.deb; do \
...
# 为所有 Kali Linux RootFS 安装 Droidspaces USB Manager
